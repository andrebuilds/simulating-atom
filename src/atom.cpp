...
struct Engine {
    ...
    Engine () {
        ...
        // --- Create Window ---
        window = glfwCreateWindow(WIDTH, HEIGHT, "2D Atom Simulation by andrebuilds", nullptr, nullptr);
        if (!window) {
                ...
                if (p.charge == -1) {
                    ...
                    for (Wave& wave : waves) {
                        for (WavePoint& wp : wave.points) {
                            ...
                            if (dist < 20.0f && abs(wave.energy - energyforUp) < 1e-4f && wave.col != vec3(1.0f, 1.0f, 0.0f)) {
                                wave.energy = 0.0f;
                                p.n += 1;
