...
double sampleR(int n, int l, mt19937& gen) {
    ...
    static int last_n = -1, last_l = -1;
    if (n != last_n || l != last_l) { built = false; last_n = n; last_l = l; }
...
double sampleTheta(int l, int m, mt19937& gen) {
    ...
    static vector<double> cdf;
    static bool built = false;
    static int last_l = -1, last_m = -1;
    if (l != last_l || m != last_m) { built = false; last_l = l; last_m = m; }
...
struct Engine {
    ...
    Engine () {
        ...
        // --- Init Window ---
        window = glfwCreateWindow(WIDTH, HEIGHT, "Quantum Simulation by andrebuilds - Raytraced", nullptr, nullptr);
